...
export class MarketDataService {
    cryptoCompareApiKey;
    baseUrl = 'https://api.cryptocompare.com/data/v2';
    supportedAssets = ['USDC', 'WBTC', 'PAXG', 'EURT'];
    lastFetchTime = 0;
    cachedData = new Map();
    cacheValidityMs = 5 * 60 * 1000; // 5 minutes
    maxCacheSize = 1000; // Prevent unlimited cache growth
    // Enhanced rate limiting
    requestCount = 0;
    maxRequestsPerHour = 100;
    lastRateLimitReset = Date.now();
    cryptoCompareRateLimit = parseInt(process.env.CRYPTOCOMPARE_RATE_LIMIT_PER_SECOND || '10');
    etherscanRateLimit = parseInt(process.env.ETHERSCAN_RATE_LIMIT_PER_SECOND || '5');
    lastApiCall = new Map();
    requestQueue = [];
     ...
     */
    async fetchPriceFromPrimarySource(symbol) {
        try {
     ...
     */
    async fetchAssetPriceWithRetry(symbol, maxRetries) {
     ...
     */
    async tryFallbackSources(symbol) {
        const coinGeckoIds = {
            'USDC': 'usd-coin',
     ...
     */
    executeWithRateLimit(provider, operation) {
    ...
    fetchAssetPrice(symbol) {
        return this.executeWithRateLimit('cryptocompare', async () => {
            try {
                ...
                throw new GovernanceError(`Failed to fetch price for ${symbol}: ${error instanceof Error ? error.message : String(error)}`, 'ASSET_PRICE_FETCH_ERROR');
            }
        });
    }
    /**
     ...
     */
    fetchHistoricalData(symbol, hours = 168) {
        return this.executeWithRateLimit('cryptocompare', async () => {
            try {
                const url = `${this.baseUrl}/histohour`;
                const params = {
                ...
                throw new GovernanceError(`Failed to fetch historical data for ${symbol}: ${error instanceof Error ? error.message : String(error)}`, 'HISTORICAL_DATA_FETCH_ERROR');
            }
        });
    }
    /**
    ...
    async analyzeMarketData() {
        try {
            ...
            const historicalData = new Map(historicalResults.map(result => [result.symbol, result.historical]));
            const analysis = {
                recommendations: {},
            ...
            throw new GovernanceError(`Market analysis failed: ${error instanceof Error ? error.message : String(error)}`, 'MARKET_ANALYSIS_ERROR');
        }
    }
    /**
     * Analyze individual asset
     */
    analyzeAsset(currentPrice, historical) {
     ...
     */
    calculateAssetRisk(currentPrice, historical) {
     ...
     */
    calculateOptimalAllocation(symbol, action, confidence) {
        ...
        const base = baseAllocations[symbol] || 25;
        if (action === 'buy') {
