...
export class DLoopGovernanceRegistration {
    ...
    initializeContracts() {
        try {
            ...
            this.aiNodeRegistryContract = new ethers.Contract(addresses.aiNodeRegistry, aiNodeRegistryAbi, this.adminWallet);
            this.dloopTokenContract = new ethers.Contract(addresses.dloopToken, dloopTokenAbi, this.adminWallet);
            this.soulboundNftContract = new ethers.Contract(addresses.soulboundNft, soulboundNftAbi, this.adminWallet);
                ...
                try {
                    ...
                    switch (approach) {
                        case 'registerAINode':
                            ...
                            tx = await this.aiNodeRegistryContract.registerAINode(nodeMetadata.endpoint, nodeMetadata.name, nodeMetadata.description, 1 // nodeType: 1 for governance nodes
                            );
                            break;
                        case 'registerNode':
                            // registerNode(address nodeAddress, address nodeOwner, string metadata)
                            tx = await this.aiNodeRegistryContract.registerNode(nodeAddress, nodeAddress, // nodeOwner is the same as nodeAddress
                            metadataJson);
                            break;
                        case 'registerNodeWithStaking':
                            // registerNodeWithStaking(address nodeAddress, string metadata, uint256 requirementId)
                            tx = await this.aiNodeRegistryContract.registerNodeWithStaking(nodeAddress, metadataJson, 0 // requirementId: 0 for default requirements
                            );
                            break;
                    }
