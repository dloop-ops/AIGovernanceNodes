...
export class GasManager {
    ...
    try {
      ...
      if (feeData.maxFeePerGas && feeData.maxPriorityFeePerGas) {
        ...
        const maxPriorityFeePerGas = BigInt(
          Math.ceil(Number(feeData.maxPriorityFeePerGas) * multiplier.price)
        );
        const maxFeePerGas = BigInt(
          Math.ceil(Number(feeData.maxFeePerGas) * multiplier.price)
        );
        ...
        const gasPrice = BigInt(
          Math.ceil(Number(networkGasPrice) * multiplier.price)
        );
  ...
  async executeWithRetry<T>(
    transactionFunction: (gasConfig: GasConfig) => Promise<T>,
    initialGasEstimate: bigint,
   ...
   */
  private async getSmartGasPrice(): Promise<bigint> {
    try {
      const feeData = await this.provider.getFeeData();
   ...
   */
  private async updateGasHistory(): Promise<void> {
    const now = Date.now();
   ...
   */
  private escalateGasLimit(baseGasLimit: bigint, attempt: number): bigint {
   ...
   */
  private escalateGasPrice(baseGasPrice: bigint, attempt: number): bigint {
  ...
  getGasMetrics(): {
    ...
    const averageGasPrice = this.gasHistory.length > 0
      ? ethers.formatUnits(
          this.gasHistory.reduce((sum, price) => sum + price, 0n) / BigInt(this.gasHistory.length),
          'gwei'
        )
      : '0';
