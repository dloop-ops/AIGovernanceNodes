...
export class DLoopGovernanceRegistration {
  ...
  private initializeContracts(): void {
    try {
      ...
      const aiNodeRegistryAbi = JSON.parse(
        fs.readFileSync(path.join(abiDir, 'ainoderegistry.abi.v1.json'), 'utf8')
      ).abi;
      const dloopTokenAbi = JSON.parse(
        fs.readFileSync(path.join(abiDir, 'dlooptoken.abi.v1.json'), 'utf8')
      ).abi;
      const soulboundNftAbi = JSON.parse(
        fs.readFileSync(path.join(abiDir, 'soulboundnft.abi.v1.json'), 'utf8')
      ).abi;
      ...
      throw new Error(
        `Failed to initialize D-Loop contracts: ${
  ...
  async registerGovernanceNode(
    ...
    try {
      ...
      if (REGISTERED_ADDRESSES.includes(nodeAddress)) {
        logger.info(
          `🛑 REGISTRATION BLOCKED: Node ${nodeIndex + 1} (${nodeAddress}) already registered`,
          {
            component: 'dloop-governance',
            nodeIndex,
            nodeAddress,
  ...
  private async verifyAdminPermissions(): Promise<void> {
    try {
      ...
      if (contractAdmin.toLowerCase() !== this.adminWallet.address.toLowerCase()) {
        throw new Error(
          `Admin wallet ${this.adminWallet.address} is not the contract admin. Contract admin is: ${contractAdmin}`
        );
      }
      ...
      try {
        ...
        const hasMinterRole = await this.soulboundNftContract.hasRole(
          minterRole,
        ...
        if (!hasMinterRole) {
          logger.warn(
            'Admin wallet does not have MINTER_ROLE on SoulboundNFT, but proceeding with registration'
          );
        }
      } catch (roleError) {
  ...
  private async ensureNodeSoulBoundNFT(nodeAddress: string): Promise<void> {
    try {
      ...
      throw new Error(
        `SoulBound NFT setup failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private async setupTokenStaking(nodeWallet: ethers.Wallet): Promise<void> {
    try {
      ...
      if (balance < stakeAmount) {
        throw new Error(
          `Insufficient DLOOP balance. Required: 1.0, Available: ${ethers.formatEther(balance)}`
        );
      }

      // Check allowance for AINodeRegistry
      const addresses = getCurrentContractAddresses();
      const allowance = await this.dloopTokenContract.allowance(
        nodeAddress,
        addresses.aiNodeRegistry
      );
      ...
      throw new Error(
        `Token staking setup failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  private async performAdminRegistration(
    ...
    try {
      ...
      const nodeMetadata = {
        ...
        description:
          'Autonomous AI agent for D-Loop protocol governance and asset rebalancing decisions',
   ...
   */
  async registerAllGovernanceNodes(): Promise<{
    registered: number;
    failed: number;
    results: any[];
  }> {
