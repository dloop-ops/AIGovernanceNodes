...
export class ConnectionPool {
  ...
  public getHealthyConnection(): ethers.JsonRpcProvider | null {
    const allConnections = Array.from(this.connections.entries()).flatMap(([name, conns]) =>
      conns.filter(conn => conn.isHealthy).map(conn => ({ ...conn, endpointName: name }))
    );
  ...
  private markConnectionUnhealthy(provider: ethers.JsonRpcProvider): void {
  ...
  private async checkConnectionHealth(endpointName: string, connection: PooledConnection): Promise<void> {
    try {
      await Promise.race([
  ...
  public getPoolStatus(): { [endpointName: string]: { healthy: number; total: number } } {
    const status: { [endpointName: string]: { healthy: number; total: number } } = {};
