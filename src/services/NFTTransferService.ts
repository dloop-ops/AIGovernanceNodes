...
export class NFTTransferService {
  ...
  async distributeSoulboundNFTs(): Promise<TransferResult[]> {
    ...
    try {
      ...
      throw new GovernanceError(
        `Failed to distribute SoulBound NFTs: ${
   ...
   */
  private async mintSoulboundNFTForNode(
    nodeId: string,
    nodeIndex: number
  ): Promise<{
    success: boolean;
   ...
   */
  private async getCurrentAuthenticationStatus(): Promise<
    Array<{
      nodeIndex: number;
      address: string;
  ...
  async verifyDistribution(): Promise<{
    ...
    try {
      ...
      throw new GovernanceError(
        `Failed to verify SoulBound NFT distribution: ${
          error instanceof Error ? error.message : String(error)
        }`,
