...
export class ContractService {
  ...
  async getProposal(proposalId: string): Promise<Proposal> {
    try {
      ...
      throw new GovernanceError(
        `Failed to get proposal ${proposalId}: ${
          error instanceof Error ? error.message : String(error)
        }`,
  ...
  async getActiveProposals(): Promise<Proposal[]> {
    try {
      ...
      for (let i = 0; i < Math.min(count, 50); i++) {
        // Limit to 50 proposals max to prevent overwhelming
        try {
          ...
          if (i > 0 && i % CHUNK_SIZE === 0) {
            logger.info(
              `Processed ${i} proposals, taking ${DELAY_BETWEEN_CHUNKS}ms break to respect rate limits`
            );
          ...
          const errorMessage =
            proposalError instanceof Error ? proposalError.message : String(proposalError);
      ...
      const errorMessage = `Failed to get active proposals: ${
        error instanceof Error ? error.message : String(error)
      }`;
  ...
  private mapProposalState(stateValue: any): ProposalState {
    ...
    switch (state) {
      ...
      case 7:
        return ProposalState.CANCELLED; // Map expired to cancelled
      default:
  ...
  private mapProposalType(typeValue: any): ProposalType {
    ...
    switch (type) {
      ...
      case 1:
        return ProposalType.DIVEST;
      case 2:
        return ProposalType.REBALANCE;
      default:
  ...
  /**
   * Safe fetch of a proposal with retry and fallback object
   */
  private async getProposalSafeWithRetry(proposalId: string, maxRetries = 3): Promise<Proposal> {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const safeState =
          typeof stateRaw === 'number' && stateRaw >= 0 && stateRaw <= 7 ? stateRaw : 0;
        ...
        return {
          ...
          assetAddress: data[5] || '',
          amount: ethers.formatEther(data[3] || 0),
          description: data[4] || `Proposal ${proposalId}`,
          votesFor: ethers.formatEther(data[6] || 0),
          votesAgainst: ethers.formatEther(data[7] || 0),
        ...
        logger.warn(
          `Attempt ${attempt}/${maxRetries} failed to fetch proposal ${proposalId}: ${
            err instanceof Error ? err.message : String(err)
          }`
        );
        if (attempt === maxRetries) {
          logger.error(`All retries failed for proposal ${proposalId}, returning fallback`);
          return {
            id: proposalId,
    ...
    // unreachable
    throw new GovernanceError(
      'Unexpected flow in getProposalSafeWithRetry',
      'PROPOSAL_FETCH_ERROR'
    );
  }
  ...
  registerAINode(
    ...
    return Promise.resolve({
      success: false,
      error:
        'REGISTRATION_BLOCKED: All 5 AI Governance Nodes are already registered and active. No additional registrations allowed.'
    });
  }
  ...
  async getNodeInfo(nodeAddress: string): Promise<NodeInfo> {
    try {
      ...
      throw new GovernanceError(
        `Failed to get node info for ${nodeAddress}: ${
          error instanceof Error ? error.message : String(error)
        }`,
  ...
  async hasVoted(proposalId: string, nodeIndex: number): Promise<boolean> {
    try {
      ...
      if (errorMessage.includes('Too Many Requests') || errorMessage.includes('rate limit')) {
        logger.warn(
          `Rate limited while checking vote status for proposal ${proposalId}, assuming already voted`,
          {
            nodeIndex,
            proposalId
          }
  ...
  async mintSoulboundNFT(nodeIndex: number, metadata: string): Promise<string> {
    try {
      ...
      throw new GovernanceError(
        `Failed to mint SoulBound NFT for node ${nodeIndex}: ${
          error instanceof Error ? error.message : String(error)
        }`,
  ...
  private async isNodeAlreadyRegistered(nodeAddress: string): Promise<boolean> {
    try {
      ...
      if (!error.message.includes('NodeNotRegistered')) {
        logger.warn('Failed to check node registration status', {
          nodeAddress,
          error: error.message
        });
      }
  ...
  async getProposals(): Promise<Proposal[]> {
    ...
    try {
      ...
      console.log(
        `📊 Found ${proposalCount} total proposals, processing in chunks of ${chunkSize}...`
      );
        ...
        for (let proposalId = i; proposalId <= chunkEnd; proposalId++) {
          try {
            ...
            if (proposal && proposal.state === ProposalState.ACTIVE) {
              proposals.push(proposal);
              console.log(
                `✅ Added active proposal ${proposalId} (${proposals.length} total active)`
              );
            }
      ...
      console.error(
        '❌ Critical error in getProposals:',
        error instanceof Error ? error.message : 'Unknown error'
      );
  ...
  private async getProposalWithRetry(
    proposalId: number,
    maxRetries: number
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        try {
          ...
          console.error(
            `❌ Error parsing proposal ${proposalId}:`,
        ...
        console.error(
          `❌ Attempt ${attempt}/${maxRetries} failed for proposal ${proposalId}:`,
          error instanceof Error ? error.message : 'Unknown error'
        );
