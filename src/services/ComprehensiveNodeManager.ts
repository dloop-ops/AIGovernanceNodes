...
import { NodeRegistrationService, NodeRegistrationConfig } from './NodeRegistrationService.js';
...
export class ComprehensiveNodeManager {
  ...
  public async initializeAndRegisterAllNodes(): Promise<boolean> {
    ...
    try {
      ...
      logger.info(
        `Node registration complete: ${successfulRegistrations}/${this.allNodeIds.length} successful`
      );
  ...
  private async processNodeRegistration(
    ...
    try {
      ...
      const isAlreadyRegistered = await this.nodeRegistrationService.checkNodeRegistration(
        config.nodeAddress
      );
  ...
  private loadNodeConfigurations(): NodeRegistrationConfig[] {
    // NUCLEAR OPTION: All nodes are already registered, skip all registration attempts
  ...
  public getNodeStatuses(): Map<string, NodeStatus> {
  ...
  public async retryFailedRegistrations(): Promise<void> {
