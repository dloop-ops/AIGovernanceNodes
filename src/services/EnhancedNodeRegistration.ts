...
export class EnhancedNodeRegistration {
  ...
  public async registerNodeWithComprehensiveFlow(config: NodeRegistrationConfig): Promise<RegistrationResult> {
  ...
  private async checkNodeRegistrationStatus(nodeAddress: string): Promise<{ isRegistered: boolean; nodeInfo?: any }> {
    try {
