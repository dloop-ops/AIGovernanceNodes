...
export class SystemStatusService {
  ...
  private logSystemHealth(status: SystemStatus): void {
    ...
    logger.info('System Health Summary', {
      ...
      rateLimitHandling: {
        successRate:
          rpc.rateLimitHandling.totalRequests > 0
            ? Math.round(
                (rpc.rateLimitHandling.successfulRequests / rpc.rateLimitHandling.totalRequests) *
                  100
              )
            : 0,
  ...
  public getMetricsSummary(): {
    ...
    const rpcReliability =
      rpc.totalEndpoints > 0 ? (rpc.activeConnections / rpc.totalEndpoints) * 100 : 0;
    ...
    const systemUptime =
      status.governanceNodes.totalNodes > 0
        ? (status.governanceNodes.activeNodes / status.governanceNodes.totalNodes) * 100
        : 0;

    const automatedRecovery =
      rpc.rateLimitHandling.rateLimitHits > 0 &&
