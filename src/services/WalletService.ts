...
export class WalletService {
    ...
    try {
      ...
      for (let i = 0; i < privateKeys.length; i++) {
        ...
        if (normalizedKey.length !== 66) {
          throw new GovernanceError(
            `Invalid private key format for AI Node ${
              i + 1
  ...
  private validatePrivateKeySecurity(privateKey: string, nodeNumber: number): void {
    ...
    if (uniqueChars < 8) {
      logger.warn(
        `Low entropy private key detected for node ${nodeNumber} (${uniqueChars} unique characters)`
      );
    }
  }
  ...
  async getBalance(nodeIndex: number): Promise<string> {
    try {
      ...
      throw new GovernanceError(
        `Failed to get balance for node ${nodeIndex}: ${
   ...
   */
  async estimateGas(nodeIndex: number, to: string, data: string, value?: string): Promise<bigint> {
    try {
      const wallet = this.getWallet(nodeIndex);
      const tx = {
        to,
