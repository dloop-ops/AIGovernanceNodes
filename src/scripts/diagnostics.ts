...
async function runDiagnostics() {
  ...
  try {
    ...
    const nodesWithIssues = results.filter(
      (r) => r.registrationErrors.length > 0 || !r.isRegistered
    );
    ...
    if (nodesWithIssues.length > 0) {
      ...
      const stillBroken = postFixResults.filter(
        (r) => r.registrationErrors.length > 0 || !r.isRegistered
      );
      ...
      if (stillBroken.length === 0) {
        ...
        console.log(
          `⚠️  ${stillBroken.length} nodes still have issues that require manual intervention:\n`
        );
