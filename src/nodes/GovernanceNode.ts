...
import { NodeConfig, GovernanceNodeState, GovernanceError } from '../types/index.js';
...
export class GovernanceNode {
  ...
  private state: GovernanceNodeState;
  ...
  async start(): Promise<void> {
    try {
      ...
      if (REGISTERED_ADDRESSES.includes(this.wallet.address)) {
        logger.info(
          `🛑 Node ${this.nodeId} - COMPLETE SKIP OF ALL REGISTRATION (already registered)`,
          {
  ...
  async checkAndVoteOnProposals(): Promise<void> {
    ...
    try {
      logger.info(`Checking and voting on proposals for node ${this.nodeId}`, {
        component: 'governance'
      });
      ...
      for (let i = 0; i < activeProposals.length; i++) {
        ...
        try {
          ...
          const txHash = await this.contractService.vote(
            this.nodeIndex,
            proposal.id,
            decision.voteSupport
          );
          votesSuccessful++;
          ...
          if (errorMessage.includes('Too Many Requests') || errorMessage.includes('rate limit')) {
            logger.warn(`Rate limited while voting, adding 5 second delay`, {
              nodeId: this.nodeId
            });
    ...
    try {
      ...
      if (ethResult.status === 'fulfilled') {
        ...
        logger.warn(`Failed to get ETH balance for node ${this.nodeId}`, {
          component: 'governance',
          error:
            ethResult.reason instanceof Error ? ethResult.reason.message : String(ethResult.reason)
        });
