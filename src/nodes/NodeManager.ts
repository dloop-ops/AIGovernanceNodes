...
export class NodeManager {
  ...
  private initializeServices(): void {
    try {
      ...
      this.soulboundNftService = new SoulboundNFTService(
        this.walletService,
        this.contractService
      );

      // Initialize NFT transfer service for SoulBound NFT distribution
      this.nftTransferService = new NFTTransferService(
        this.contractService,
        this.walletService
      );

      // Initialize D-Loop governance registration service
      this.dloopRegistrationService = new DLoopGovernanceRegistration(
        this.walletService
      );
  ...
  private initializeNodes(configs: NodeConfig[]): void {
    ...
    for (const config of configs) {
      try {
        ...
        throw new GovernanceError(
          `Node initialization failed for ${config.id}: ${error instanceof Error ? error.message : String(error)}`,
          'NODE_INIT_ERROR'
        );
      }
    }
  ...
  private async performHealthCheck(): Promise<void> {
    try {
      ...
      const recentlyActive = nodeStatuses.filter(status => 
        Date.now() - Math.max(status.stats.lastProposalTime, status.stats.lastVoteTime) < 24 * 60 * 60 * 1000
      ).length;
  ...
  async restartNode(nodeId: string): Promise<void> {
    ...
    try {
      ...
      throw new GovernanceError(
        `Failed to restart node ${nodeId}: ${error instanceof Error ? error.message : String(error)}`,
        'NODE_RESTART_ERROR'
      );
    }
  }
