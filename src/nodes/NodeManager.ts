...
export class NodeManager {
  private nodes: Map<string, GovernanceNode> = new Map();
  ...
  async start(): Promise<void> {
    try {
      ...
      throw new GovernanceError(
        `Failed to start Node Manager: ${error instanceof Error ? error.message : String(error)}`,
        'NODE_MANAGER_START_ERROR'
      );
    }
  }
   ...
   * Load node configurations
   */
  private loadNodeConfigurations(): NodeConfig[] {
    ...
    logger.info(
      `Loaded ${enabledConfigs.length} enabled node configurations out of ${configs.length} total`
    );
   ...
   */
  private initializeNodes(configs: NodeConfig[]): void {
    logger.info(`Initializing ${configs.length} governance nodes`);

    for (const config of configs) {
      try {
        const node = new GovernanceNode(
          config,
          this.walletService,
        ...
        throw new GovernanceError(
          `Node initialization failed for ${config.id}: ${
   ...
   */
  private registerGovernanceNodes(): void {
   ...
   */
  private async stopAllNodes(): Promise<void> {
  ...
  private setupScheduledTasks(): void {
    ...
    this.scheduler.addTask(
      proposalCreationTask.name,
      proposalCreationTask.schedule,
      proposalCreationTask.task
    );
    ...
    this.scheduler.addTask(
      tokenMonitoringTask.name,
      tokenMonitoringTask.schedule,
      tokenMonitoringTask.task
    );
    this.scheduler.addTask(
      authenticationTask.name,
      authenticationTask.schedule,
   ...
   */
  private async executeVotingRound(): Promise<void> {
  ...
  private async performHealthCheck(): Promise<void> {
    try {
      ...
      const recentlyActive = nodeStatuses.filter(
        (status) =>
          Date.now() - Math.max(status.stats.lastProposalTime, status.stats.lastVoteTime) <
          24 * 60 * 60 * 1000
      ).length;
   ...
   */
  private async performTokenChecks(): Promise<void> {
    try {
   ...
   */
  private performAuthenticationChecks(): void {
  ...
  private async logSystemStatus(): Promise<void> {
    try {
      ...
      const totalProposals = nodeStatuses.reduce(
        (sum, status) => sum + status.stats.proposalsCreated,
        0
      );
   ...
   */
  async triggerVotingRound(): Promise<void> {
