 ...
 */
export const treasuryStatusAction: TreasuryAction = {
...
export const treasuryProvider = {
  ...
  get: async (runtime: any, message: any) => {
    try {
      ...
      const healthyNodes = nodeBalances.filter((n: any) => n.balance > 100).length;
...
📋 **Node Balances**:
${nodeBalances.map((n: any) => 
  `• Node ${n.nodeIndex}: ${n.formattedBalance}`
).join('\n')}
